...
The cmake script checks for the availability of these libraries by looking for the expected folders under `externals`. For example, `osg` and `sumo` are checked through their corresponding folders, while `osi` and `implot` are built, released and version controlled via tags, so cmake maps the selected version to a release tag and looks for the matching tagged package locally under `externals/osi` and `externals/implot`. If a required package is missing, cmake automatically downloads and extracts it.
...
`osi` and `implot` accepts explicit version selection via the flags `-D OSI_VERSION` and `-D IMPLOT_VERSION` respectively. Supported versions can be found in https://github.com/esmini/esmini/blob/master/support/cmake/common/version_mapping.cmake.
