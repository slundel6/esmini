...
Example scenario https://github.com/esmini/esmini/blob/dev/EnvironmentSimulator/Unittest/xosc/race_hid.xosc[race_hid.xosc] shows how to assign game controllers to one or several vehicles. The scenario puts three cars at beginning of a road. One will immediatelly start driving. The second is controlled by a game controller, which needs to be setup for specific connected device - see details below. The third can also be controlled by a game controller by uncommenting the ObjectController code in the sceenario. Otherwise, by default it will stand still. First car reaching end of road is the winner.
...
A examples are provided in the separate catalog https://github.com/esmini/esmini/blob/dev/resources/xosc/Catalogs/Controllers/ControllerCatalog.xosc[ControllerCatalog.xosc], including:
...
.Mapping
++++
<iframe src="https://docs.google.com/spreadsheets/d/e/2PACX-1vTIxA9xhO1GomQDd3GvamIaul80S4GLH4UHyWubsG7sm3lLuCMqMo0QcHg-6D-NfKqqFKDwGWa1W_g2/pubhtml?gid=2029882313&amp;single=true&amp;widget=false&amp;headers=false&amp" width="550" height="1865"></iframe>
++++
