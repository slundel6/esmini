...
=== Environment Action
This action is to dynamically control environmental conditions, It allows you to define and modify weather, road conditions, and time of day, creating realistic and varied testing environments.

The EnvironmentAction is used within GlobalAction elements in your OpenSCENARIO file. You can define a new environment directly within the EnvironmentAction or reference a pre-defined environment from a catalog.

Two ways to define EnvironmentAction:

- **Directly in the OpenSCENARIO file**:
You can define the environment directly within the EnvironmentAction element in your OpenSCENARIO file. This allows you to specify the desired weather, road conditions, and time of day for that specific scenario.
    ...
    <EnvironmentAction>
        <Environment name="weather">
            ...
            <Weather cloudState="free" temperature="300" atmosphericPressure="10000">
                <Sun azimuth="0.4" intensity="10000" elevation="0.3"/>
...
- **Using a Catalog**:
You can create a catalog of predefined environments and reference them in your OpenSCENARIO file. This allows you to reuse the same environment settings across multiple scenarios, ensuring consistency and reducing redundancy.
...
<GlobalAction>
    <EnvironmentAction>
        <Environment name="weather">
            <CatalogReference catalogName="WeatherCatalog" catalogVersion="summer"/>
        </Environment>
    </EnvironmentAction>
</GlobalAction>
----
The catalog reference points to a specific environment defined in the WeatherCatalog which is placed in resources/xosc/Catalogs/Environments. The catalog can contain multiple environments, each with its own set of parameters.

**NOTES**

- TimeOfDay animation is not supported in the viewer. The viewer will only display the sky color corresponding to the set time of day. OSI will synchronize the time of day with the simulation time and update accordingly only for animation true.
- Rain, Snow, wind cloud cover and precipitation not visualized in the viewer. The OSI will still populate those details, but the visual representation of those will not be shown.
...
OSI groundtruth reported via the API always contains both static and dynamic objects in the first frame. By default subsequent frames will only contain dynamic objects. The same data is also stored in the OSI log file, given that the log file is enabled.

However, the behavior can be configured as follows:

The dynamic objects are reported every frame. How the static objects are reported and logged can be controlled by `SE_SetOSIStaticReportMode` function. The available options are according to the enum `SE_OSIStaticReportMode`:
