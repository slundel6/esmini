...
TEST(GetOSIRoadLaneTest, lane_no_obj)
{
    ...
    EXPECT_EQ(fileStatus.st_size, 88170);  // initial OSI size, including static content

    SE_StepDT(0.001);
    SE_FlushOSIFile();
    ASSERT_EQ(stat("gt.osi", &fileStatus), 0);
    EXPECT_EQ(fileStatus.st_size, 89758);  // slight growth due to only dynamic updates
    ...
    SE_FlushOSIFile();
    ASSERT_EQ(stat("gt.osi", &fileStatus), 0);
    EXPECT_EQ(fileStatus.st_size, 91346);  // slight growth due to only dynamic updates

    SE_StepDT(0.001);  // Step for write another frame to osi file
    SE_FlushOSIFile();
    ASSERT_EQ(stat("gt.osi", &fileStatus), 0);
    EXPECT_EQ(fileStatus.st_size, 92935);  // slight growth due to only dynamic updates
