...
osg::ref_ptr<osg::Node> Viewer::CreateShadow(double bb_x, double bb_y, double bb_z)
{
    ...
    int fan1_start = makeExtrusionFan(osg::PI, -LENGTH, -WIDTH);             // BL
    int fan2_start = makeExtrusionFan(3.0 * osg::PI / 2.0, LENGTH, -WIDTH);  // BR
    int fan3_start = makeExtrusionFan(0.0, LENGTH, WIDTH);                   // TR
    int fan4_start = makeExtrusionFan(osg::PI / 2.0, -LENGTH, WIDTH);        // TL
